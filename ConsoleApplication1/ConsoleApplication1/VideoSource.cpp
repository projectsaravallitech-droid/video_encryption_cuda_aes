...
#include "VideoSource.hpp"
    ...
    std::unique_ptr<VideoSource> CameraFactory:: createSource(CameraFactory::Platform type, const std::string& path) {
        switch (type) {
        case Platform::Windows:
            return std::make_unique<WindowsCamera>(0);
        case Platform::Linux:
            return std::make_unique<LinuxCamera>(0);
        case Platform::File:
            return std::make_unique<VideoFile>("filename");
        default:
            return nullptr;
        }
    }
